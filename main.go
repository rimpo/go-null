...
func (g *Generator) appendInAllType(typeName string) error {
	...
	f, err := os.OpenFile(nullDir+"/all_types.txt", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return err
	}
...
func (g *Generator) createJsonxLib() error {
	...
	if err := ioutil.WriteFile(jsonxDir+"/jsonx.go", buf.Bytes(), 0644); err != nil {
	...
	if err := ioutil.WriteFile(jsonxDir+"/jsonx_test.go", buf.Bytes(), 0644); err != nil {
...
func (g *Generator) createNullForStringEnum(typ *Type) {
	...
	var buf bytes.Buffer
	fmt.Fprintf(&buf, basicHeaderCode)
	...
	t = template.Must(template.New("is_safe_value").Parse(generateEnumSwitch(typ.values)))
	err = t.Execute(&buf, struct {
		TypeName        string
		BuiltInTypeName string
	}{
	...
	if err := ioutil.WriteFile(typeFilePath, buf.Bytes(), 0644); err != nil {
