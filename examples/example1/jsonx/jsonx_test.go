...
import (
	...
	"gitlab.com/shaadi/profile-api/pkg/typ"
)
...
func fillDummyData(p *testProfile) {
	...
	ph.Url.Set("./test/abc")
	ph.ID.Set(1)
	ph.Sizes = make(map[string]string)
	ph.Sizes["20x20"] = "asdklfjksd"
	ph.Sizes["20x40"] = "949203840"
	p.PhotoDetails.Photos = append(p.PhotoDetails.Photos, ph)
	ph.Url.Set("./test/xyz")
	ph.ID.Set(2)
	ph.Sizes = make(map[string]string)
	ph.Sizes["30x20"] = "asdklfjksd"
	ph.Sizes["40x40"] = "949203840"
	p.PhotoDetails.Photos = append(p.PhotoDetails.Photos, ph)
